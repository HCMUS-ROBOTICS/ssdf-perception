...
class LaneLine {
 public:
  ...
  inline LaneParams getParams() const { return this->params; }

  inline cv::Mat getLineImage() const { return this->lineImage; }

  inline cv::Rect getBirdviewRect() const {
    return cv::Rect2i(0, 0, this->lineImage.cols - 1, this->lineImage.rows - 1);
  }

  inline bool isFound() const { return !this->points.empty(); }

  LanePoints getPoints() const { return this->points; }
...
class LeftLane : public LaneLine {
 public:
  ...
  virtual std::string getName() const override { return "Left"; }
};
...
class RightLane : public LaneLine {
 public:
  ...
  virtual std::string getName() const override { return "Right"; }
};

class WindowExtractImpl : public IExtractorImpl {
 public:
  WindowExtractImpl();

  virtual void update() override;

  virtual LaneParams getLeftParams() const override { return this->left.getParams(); }
  virtual LaneParams getRightParams() const override { return this->right.getParams(); }
