...
WindowExtractImpl::WindowExtractImpl() : _imageTransport{_nh} {
...
void WindowExtractImpl::update() {
  ...
  if (IS_USE_BIRDVIEW) {
    ...
    std::tie(lineImage, birdTransform) = birdviewTransformation(
        this->_laneSegImage, birdwidth, birdheight, skyline, offsetLeft, offsetRight);
...
bool LaneLine::updateNewCenter(const cv::Mat &region, cv::Point &center,
  ...
  cv::Rect roiTracking{center.x - W_TRACKING / 2, center.y - H_TRACKING / 2, W_TRACKING,
                       H_TRACKING};
...
bool LaneLine::findBeginPoint(cv::Point &returnPoint) const {
  ...
  for (int center_y = region.rows - 1 - H_TRACKING / 2; center_y > H_TRACKING / 2 + 1;
       center_y -= H_TRACKING) {
    int maxNonZero = 0;
    cv::Point maxPoint;

    for (int center_x = W_TRACKING / 2 + 1; center_x < region.cols - W_TRACKING / 2 - 1;
         center_x += W_TRACKING) {
